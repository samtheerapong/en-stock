	...
	*/
	public function fetchBrandData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if(in_array('viewBrand', $this->permission)) {
				$buttons .= '<button type="button" class="btn btn-warning btn-sm" onclick="editBrand('.$value['id'].')" data-toggle="modal" data-target="#editBrandModal"><i class="fa fa-pencil"></i> แก้ไข</button>';	
			}
			
			if(in_array('deleteBrand', $this->permission)) {
				$buttons .= ' <button type="button" class="btn btn-danger btn-sm" onclick="removeBrand('.$value['id'].')" data-toggle="modal" data-target="#removeBrandModal"><i class="fa fa-trash"></i> ลบ</button>
				';
			}				
	...
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	if($create == true) {
        		...
        		$response['messages'] = 'เพิ่มข้อมูล สำเร็จ';
        	}
        	else {
	...
	public function update($id)
	{
		...
		if($id) {
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	...
	        	if($update == true) {
	        		$response['success'] = true;
	        		$response['messages'] = 'แก้ไขข้อมูล สำเร็จ';
	        	}
	        	else {
	        		$response['success'] = false;
