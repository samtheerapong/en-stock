	...
	*/
	public function fetchAttributeData()
	{
		...
		foreach ($data as $key => $value) {
			...
			// button
			$buttons = '<a href="'.base_url('attributes/addvalue/'.$value['id']).'" class="btn btn-primary btn-sm"><i class="fa fa-database"></i> จัดการชั้นวาง</a> 
			<button type="button" class="btn btn-warning btn-sm" onclick="editFunc('.$value['id'].')" data-toggle="modal" data-target="#editModal"><i class="fa fa-edit"></i> แก้ไข</button>
			<button type="button" class="btn btn-danger btn-sm" onclick="removeFunc('.$value['id'].')" data-toggle="modal" data-target="#removeModal"><i class="fa fa-trash"></i> ลบ</button>
			';

			$status = ($value['active'] == 1) ? '<span class="label label-success">ใช้งาน</span>' : '<span class="label label-danger">ไม่ใช้งาน</span>';
	...
	*/
	public function fetchAttributeValueData($attribute_parent_id)
	{
		...
		foreach ($data as $key => $value) {
			...
			// button
			$buttons = '
			<button type="button" class="btn btn-warning btn-sm" onclick="editFunc('.$value['id'].')" data-toggle="modal" data-target="#editModal"><i class="fa fa-pencil"></i> แก้ไข</button>
			<button type="button" class="btn btn-danger btn-sm" onclick="removeFunc('.$value['id'].')" data-toggle="modal" data-target="#removeModal"><i class="fa fa-trash"></i> ลบ</button>
			';
